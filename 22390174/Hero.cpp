...
//Getter κατοχής κλειδιού από τον ήρωα
bool Hero::getHasKey() const {
    return hasKey;
}

//Setter κατοχής κλειδιού από τον ήρωα
void Hero::setHasKey(bool value) {
    hasKey = value;
}

//Getter αν ο ήρωας έχει πιαστεί σε παγίδα
bool Hero::getIsTrapped() const {
    return isTrapped;
}

//Setter αν ο ήρωας έχει πιαστεί σε παγίδα
void Hero::setIsTrapped() {
    isTrapped = true;
}

//Getter συμβόλου ήρωα
...
std::vector<std::pair<int, int>> Hero::route_to_L(int startRow, int startCol, int endRow, int endCol, Map& map) {
