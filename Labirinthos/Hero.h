...
class Hero {
   ...
   public:
	...
	bool hasVisitedCell(int row,int col);           //Ελέγχοι αν έχει ξαναβρεθεί στη συγκεκριμένη τοποθεσία
        ...
        std::vector<std::pair<int, int>> bfsToTarget(int startRow, int startCol, int endRow, int endCol, Map& map); 
	...
	virtual void move(Map& map, Key& key,Ladder& ladder, Trap& trap1, Trap& trap2)=0;				//
};
