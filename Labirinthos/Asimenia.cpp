...
void Asimenia::move(Map& map,Grigorakis& grigorakis, Key& key,Ladder& ladder, Trap& trap1, Trap& trap2) 
{
    ...
    if(!map.get_state() && !map.Getdestroyedwalls())
    {
        ...
        for (const std::pair<int, int>& cell : visible) 
        {
            ...
            // 5. Έλεγχος για παγίδα που έχει πίασει τον Γρηγοράκη

            if (map.getCell(row, col) == 'C') 
            {
                if (key.getTaken() && !grigorakis.getHasKey()) 
                {
        ...
        if(!(get_foundC() && getHasKey()))
        {
            ...
            //Αν το κλειδί έχει παρθεί γενικότερα και δεν το έχει ο Γρηγοράκης, τότε η Ασημένια έχει το κλειδί
            //Αναγκαστικά μπήκε αυτή η συνθήκη, γιατί η συνάρτηση από μόνη της όταν ήταν στην if που ελέγχω για
            //το κλειδί προκαλούσε segmentation fault.
            if(key.getTaken() && !grigorakis.getHasKey())
            {
                setHasKey(true);
            }
    ...
    {   
        ...
        if(ladder.getVisible())
        {
            if(getrouteHL())
            {
                route_H_L = bfsToTarget(currentRow,currentCol,ladder.getRow(),ladder.getCol(),map);  
                setrouteHL(false);
            }
        }
    ...
    //τον λαβύρινθο)
    if(getRow() == ladder.getRow() && getCol() == ladder.getCol())
    {
        map.setCell(ladder.getRow(),ladder.getCol(),'L');
    }
